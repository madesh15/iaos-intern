"""
Cybersecurity & Information Security module — API router.
...
MANIFEST = {
    ...
    "description": "Assesses cyber-control posture: perimeter/endpoint security, vulnerability management, IR and DLP.",
    "icon": "shield",
...
DEFAULT_PROCEDURES = [
    (1, "Perimeter & Firewall Review", "Network boundary controls"),
    (2, "Endpoint & Antivirus Coverage", "Device protection status"),
    (3, "Vulnerability Scan Results", "Open vulnerabilities tracking"),
    (4, "Penetration-Test Follow-up", "VAPT finding closure"),
    (5, "Incident-Response Readiness", "IR plan and drill evidence"),
    (6, "Data-Loss Prevention (DLP)", "Exfiltration controls"),
    (7, "Email & Phishing Defence", "Anti-phishing and awareness"),
    (8, "Encryption at Rest / Transit", "Data-encryption coverage"),
    (9, "Security-Log Monitoring (SIEM)", "Alerting and correlation"),
    (10, "Multi-Factor Authentication", "MFA coverage on key systems"),
    (11, "Third-Party / Cloud Security", "Vendor and SaaS posture"),
    (12, "Security-Patch Timeliness", "Critical-patch SLA"),
    (13, "Access to Sensitive Data", "PII/financial-data controls"),
    (14, "Security-Awareness Training", "Staff training completion"),
    (15, "Breach & Ransomware Preparedness", "Recovery and containment"),
]


def _ensure_seeded(db: DbSession, current_user: CurrentUser) -> None:
    existing = tenant_scoped(db.query(m.CybersecurityProcedure), current_user).count()
    if existing:
        return
    for step_no, title, desc in DEFAULT_PROCEDURES:
        db.add(
            m.CybersecurityProcedure(
...
def list_procedures(current_user: CurrentUser, db: DbSession):
    ...
    q = tenant_scoped(db.query(m.CybersecurityProcedure), current_user).order_by(m.CybersecurityProcedure.step_no)
...
def update_procedure(proc_id: int, payload: s.ProcedureUpdate, current_user: CurrentUser, db: DbSession):
    proc = _get_or_404(db, current_user, m.CybersecurityProcedure, proc_id)
...
def list_scope(current_user: CurrentUser, db: DbSession):
    q = tenant_scoped(db.query(m.CybersecurityScope), current_user)
...
def create_scope(payload: s.ScopeIn, current_user: CurrentUser, db: DbSession):
    item = m.CybersecurityScope(tenant_id=current_user.tenant_id, **payload.model_dump())
    db.add(item)
    db.commit()
    db.refresh(item)
...
def delete_scope(item_id: int, current_user: CurrentUser, db: DbSession):
    item = _get_or_404(db, current_user, m.CybersecurityScope, item_id)
    db.delete(item)
    db.commit()
    return {"ok": True}
...
def list_rcm(current_user: CurrentUser, db: DbSession):
    q = tenant_scoped(db.query(m.CybersecurityRiskControl), current_user)
...
def create_rcm(payload: s.RiskControlIn, current_user: CurrentUser, db: DbSession):
    item = m.CybersecurityRiskControl(tenant_id=current_user.tenant_id, **payload.model_dump())
    db.add(item)
    db.commit()
    db.refresh(item)
...
def delete_rcm(item_id: int, current_user: CurrentUser, db: DbSession):
    item = _get_or_404(db, current_user, m.CybersecurityRiskControl, item_id)
    db.delete(item)
    db.commit()
    return {"ok": True}
...
def list_rules(current_user: CurrentUser, db: DbSession):
    q = tenant_scoped(db.query(m.CybersecurityTestRule), current_user)
...
def create_rule(payload: s.TestRuleIn, current_user: CurrentUser, db: DbSession):
    item = m.CybersecurityTestRule(tenant_id=current_user.tenant_id, **payload.model_dump())
    db.add(item)
    db.commit()
    db.refresh(item)
...
def delete_rule(item_id: int, current_user: CurrentUser, db: DbSession):
    item = _get_or_404(db, current_user, m.CybersecurityTestRule, item_id)
    db.delete(item)
    db.commit()
    return {"ok": True}
...
def list_datasources(current_user: CurrentUser, db: DbSession):
    q = tenant_scoped(db.query(m.CybersecurityDataSource), current_user)
...
def create_datasource(payload: s.DataSourceIn, current_user: CurrentUser, db: DbSession):
    item = m.CybersecurityDataSource(tenant_id=current_user.tenant_id, **payload.model_dump())
    db.add(item)
    db.commit()
    db.refresh(item)
...
def delete_datasource(item_id: int, current_user: CurrentUser, db: DbSession):
    item = _get_or_404(db, current_user, m.CybersecurityDataSource, item_id)
    db.delete(item)
    db.commit()
    return {"ok": True}
...
def list_samples(current_user: CurrentUser, db: DbSession):
    q = tenant_scoped(db.query(m.CybersecuritySample), current_user)
...
def create_sample(payload: s.SampleIn, current_user: CurrentUser, db: DbSession):
    item = m.CybersecuritySample(tenant_id=current_user.tenant_id, **payload.model_dump())
    db.add(item)
    db.commit()
    db.refresh(item)
...
def delete_sample(item_id: int, current_user: CurrentUser, db: DbSession):
    item = _get_or_404(db, current_user, m.CybersecuritySample, item_id)
    db.delete(item)
    db.commit()
    return {"ok": True}
...
def list_exceptions(current_user: CurrentUser, db: DbSession):
    q = tenant_scoped(db.query(m.CybersecurityException), current_user).order_by(m.CybersecurityException.created_at.desc())
...
def create_exception(payload: s.ExceptionIn, current_user: CurrentUser, db: DbSession):
    item = m.CybersecurityException(tenant_id=current_user.tenant_id, **payload.model_dump())
    db.add(item)
    db.commit()
    db.refresh(item)
...
def update_exception(item_id: int, payload: s.ExceptionIn, current_user: CurrentUser, db: DbSession):
    item = _get_or_404(db, current_user, m.CybersecurityException, item_id)
    for field, value in payload.model_dump(exclude_unset=True).items():
...
def list_evidence(current_user: CurrentUser, db: DbSession):
    q = tenant_scoped(db.query(m.CybersecurityEvidence), current_user).order_by(m.CybersecurityEvidence.created_at.desc())
...
def create_evidence(payload: s.EvidenceIn, current_user: CurrentUser, db: DbSession):
    item = m.CybersecurityEvidence(tenant_id=current_user.tenant_id, **payload.model_dump())
    db.add(item)
    db.commit()
    db.refresh(item)
...
def delete_evidence(item_id: int, current_user: CurrentUser, db: DbSession):
    item = _get_or_404(db, current_user, m.CybersecurityEvidence, item_id)
    db.delete(item)
    db.commit()
    return {"ok": True}
...
def list_findings(current_user: CurrentUser, db: DbSession):
    q = tenant_scoped(db.query(m.CybersecurityFinding), current_user).order_by(m.CybersecurityFinding.created_at.desc())
...
def create_finding(payload: s.FindingIn, current_user: CurrentUser, db: DbSession):
    item = m.CybersecurityFinding(tenant_id=current_user.tenant_id, **payload.model_dump())
    db.add(item)
    db.commit()
    db.refresh(item)
...
def update_finding(item_id: int, payload: s.FindingIn, current_user: CurrentUser, db: DbSession):
    item = _get_or_404(db, current_user, m.CybersecurityFinding, item_id)
    for field, value in payload.model_dump(exclude_unset=True).items():
        setattr(item, field, value)
    db.commit()
    db.refresh(item)
...
def list_actions(current_user: CurrentUser, db: DbSession):
    q = tenant_scoped(db.query(m.CybersecurityAction), current_user).order_by(m.CybersecurityAction.created_at.desc())
...
def create_action(payload: s.ActionIn, current_user: CurrentUser, db: DbSession):
    item = m.CybersecurityAction(tenant_id=current_user.tenant_id, **payload.model_dump())
    db.add(item)
    db.commit()
    db.refresh(item)
...
def update_action(item_id: int, payload: s.ActionIn, current_user: CurrentUser, db: DbSession):
    item = _get_or_404(db, current_user, m.CybersecurityAction, item_id)
    for field, value in payload.model_dump(exclude_unset=True).items():
        setattr(item, field, value)
    db.commit()
    db.refresh(item)
...
def dashboard(current_user: CurrentUser, db: DbSession):
    ...
    procedures = tenant_scoped(db.query(m.CybersecurityProcedure), current_user).all()
    total = len(procedures)
    completed = sum(1 for p in procedures if p.status == "completed")
    coverage = round((completed / total) * 100, 1) if total else 0.0

    open_exceptions = tenant_scoped(db.query(m.CybersecurityException), current_user).filter(
        m.CybersecurityException.status != "closed"
    ).count()
    open_findings = tenant_scoped(db.query(m.CybersecurityFinding), current_user).filter(
        m.CybersecurityFinding.status != "closed"
    ).count()
    open_actions = tenant_scoped(db.query(m.CybersecurityAction), current_user).filter(
        m.CybersecurityAction.status != "done"
    ).count()

    high_findings = tenant_scoped(db.query(m.CybersecurityFinding), current_user).filter(
        m.CybersecurityFinding.status != "closed",
        m.CybersecurityFinding.severity.in_(["high", "critical"]),
    ).count()
